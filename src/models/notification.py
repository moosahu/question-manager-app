...
class Notification(db.Model):
    ...
    # العلاقات
    student_notifications = db.relationship('StudentNotification', backref='notification', 
    ...
    @staticmethod
    def create_notification(title, body, notification_type='info', created_by_admin=False, 
                           created_by_ai=False, data=None, admin_id=None, ai_analysis_id=None):
        """إنشاء إشعار جديد"""
...
class StudentNotification(db.Model):
    ...
    # العلاقات
    student = db.relationship('Student', backref=db.backref('notifications', lazy='dynamic'))
    
    # القيود
    __table_args__ = (
        db.UniqueConstraint('notification_id', 'student_id', name='unique_notification_student'),
    )
    
    def __repr__(self):
        return f'<StudentNotification {self.id}: N{self.notification_id} → S{self.student_id}>'
    ...
    def to_dict(self):
        ...
        return {
            ...
            'notification_type': self.notification.notification_type if self.notification else None,
            'is_read': self.is_read,
            'read_at': self.read_at.isoformat() if self.read_at else None,
            'created_at': self.created_at.isoformat() if self.created_at else None,
        }
    ...
    def create_for_students(notification_id, student_ids):
        ...
        for student_id in student_ids:
            sn = StudentNotification.create_for_student(notification_id, student_id)
            if sn:
                created.append(sn)
        return created
    ...
    def get_student_notifications(student_id, unread_only=False, limit=50):
        ...
        return query.order_by(StudentNotification.created_at.desc()).limit(limit).all()
