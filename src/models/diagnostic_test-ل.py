...
class DiagnosticTest(db.Model):
    ...
    scheduled_start = db.Column(db.DateTime, nullable=True)  # متى يبدأ الاختبار
    scheduled_end = db.Column(db.DateTime, nullable=True)    # متى ينتهي الاختبار
    
    # الطلاب المخصص لهم الاختبار
    assigned_students = db.Column(db.JSON, default=[])  # قائمة IDs الطلاب
    
    # تخصيص حسب الصف (اختياري)
    assigned_grade = db.Column(db.String(50), nullable=True)  # أول/ثاني/ثالث ثانوي
    
    # حالة الجدولة
    is_scheduled = db.Column(db.Boolean, default=False)  # هل الاختبار مجدول؟
    schedule_status = db.Column(db.String(20), default='pending')  # pending, active, expired
    
    # إشعارات
    notification_sent = db.Column(db.Boolean, default=False)  # هل تم إرسال إشعار؟
    ...
    def is_available_now(self):
        ...
        if not self.is_scheduled:
            return True  # غير مجدول = متاح دائماً
    ...
    def is_student_assigned(self, student_id):
        ...
        if not self.is_scheduled:
            return True  # غير مجدول = متاح للجميع
...
class DiagnosticResult(db.Model):
    ...
    # الطالب
    student_id = db.Column(db.Integer, nullable=True)  # من جدول students
    student_name = db.Column(db.String(255), nullable=True)
    device_id = db.Column(db.String(255), nullable=True)
    
    # النتيجة
    score = db.Column(db.Integer, default=0)  # عدد الإجابات الصحيحة
    total_questions = db.Column(db.Integer, default=0)
    score_percentage = db.Column(db.Float, default=0)  # النسبة المئوية
    ...
    weak_topics = db.Column(db.JSON, default=[])   # المواضيع الضعيفة
    strong_topics = db.Column(db.JSON, default=[])  # المواضيع القوية
