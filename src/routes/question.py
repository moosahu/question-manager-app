# src/routes/question.py (Modified for Cloudinary, Import & Template Download with Debug)
...
debug_logger = logging.getLogger('question_import_debug')
...
# --- START: Import Questions Route (Modified with Debug) --- #
@question_bp.route("/import", methods=["GET", "POST"])
@login_required
def import_questions():
    debug_logger.info("==== بدء استيراد الأسئلة مع تسجيل تصحيح مفصل ====")
    
    # تسجيل معلومات الطلب
    debug_logger.info(f"طريقة الطلب: {request.method}")
    
    lessons = get_sorted_lessons()
    if not lessons:
        debug_logger.warning("لا توجد دروس متاحة للاستيراد")
        flash("لا يمكن استيراد الأسئلة. يرجى إضافة المناهج (الدورات والوحدات والدروس) أولاً.", "warning")
        return redirect(url_for("curriculum.list_courses"))

    if request.method == "POST":
        debug_logger.info("تم استلام طلب POST لاستيراد الأسئلة")
        
        # تسجيل معلومات النموذج
        form_data = {}
        for key in request.form:
            form_data[key] = request.form[key]
        debug_logger.info(f"بيانات النموذج: {form_data}")
        ...
        for key in request.files:
            ...
            if file and file.filename:
                files_info[key] = {
                    ...
                    'size': len(file.read()) if hasattr(file, 'read') else 'غير متاح'
                }
                # إعادة مؤشر القراءة إلى البداية
                if hasattr(file, 'seek'):
                    file.seek(0)
            else:
                files_info[key] = 'ملف فارغ أو غير موجود'
        debug_logger.info(f"معلومات الملفات: {files_info}")
        
        lesson_id = request.form.get("lesson_id")
        debug_logger.info(f"معرف الدرس المختار: {lesson_id}")
        
        file = request.files.get("question_file")
        debug_logger.info(f"ملف الأسئلة: {file.filename if file and file.filename else 'غير موجود'}")

        if not lesson_id:
            debug_logger.warning("لم يتم اختيار درس")
        ...
        if not file or file.filename == "":
            debug_logger.warning("لم يتم اختيار ملف")
        ...
        if not allowed_import_file(file.filename):
            debug_logger.warning(f"نوع الملف غير مسموح به: {file.filename}")
            flash("نوع الملف غير مسموح به. يرجى استخدام ملفات Excel (.xlsx) أو CSV (.csv).", "danger")
            return render_template("question/import_questions.html", lessons=lessons, selected_lesson_id=lesson_id)
        
        try:
            debug_logger.info(f"محاولة قراءة الملف المرفوع: {file.filename}")
            # تسجيل محتوى الملف للتصحيح (أول 10 سطور)
            try:
                # حفظ الموضع الحالي
                ...
                # قراءة وتسجيل المحتوى
                content = file.read()
                debug_logger.info(f"حجم محتوى الملف: {len(content)} بايت")
                
                # إعادة المؤشر إلى الموضع الأصلي
                if hasattr(file, 'seek'):
                    file.seek(0)
            except Exception as e:
                debug_logger.error(f"خطأ أثناء قراءة محتوى الملف: {e}")
            
            file_content = io.BytesIO(file.read())
            debug_logger.info(f"تم قراءة الملف إلى ذاكرة مؤقتة، حجم البيانات: {file_content.getbuffer().nbytes} بايت")
            file_content.seek(0)
            
            if file.filename.endswith(".xlsx"):
                debug_logger.info("محاولة قراءة ملف Excel")
                try:
                    df = pd.read_excel(file_content, engine='openpyxl')
                    debug_logger.info(f"تم قراءة ملف Excel بنجاح. الأبعاد: {df.shape}")
                except Exception as excel_err:
                    debug_logger.error(f"خطأ في قراءة ملف Excel: {excel_err}")
                    raise
            else:
                debug_logger.info("محاولة قراءة ملف CSV")
                try:
                    df = pd.read_csv(file_content, encoding='utf-8')
                    debug_logger.info(f"تم قراءة ملف CSV بنجاح (UTF-8). الأبعاد: {df.shape}")
                except UnicodeDecodeError:
                    debug_logger.warning("فشل ترميز UTF-8، محاولة استخدام cp1256 لملف CSV.")
                    file_content.seek(0)
                    try:
                        df = pd.read_csv(file_content, encoding='cp1256')
                        debug_logger.info(f"تم قراءة ملف CSV بنجاح (cp1256). الأبعاد: {df.shape}")
                    except Exception as cp_err:
                        debug_logger.error(f"خطأ في قراءة ملف CSV بترميز cp1256: {cp_err}")
                        raise
                except Exception as csv_err:
                    debug_logger.error(f"خطأ في قراءة ملف CSV: {csv_err}")
                    raise
            
            debug_logger.info(f"الأعمدة الموجودة في الملف: {df.columns.tolist()}")
            df.columns = [str(col).strip().lower().replace(" ", "_") for col in df.columns]
            debug_logger.info(f"الأعمدة بعد التوحيد: {df.columns.tolist()}")
            
            # استخدام الأعمدة المتوقعة المحددة عالمياً
            expected_columns_lower = [col.lower().replace(" ", "_") for col in EXPECTED_IMPORT_COLUMNS]
            debug_logger.info(f"الأعمدة المتوقعة بعد التوحيد: {expected_columns_lower}")
            
            missing_cols = [col for col in expected_columns_lower if col not in df.columns]
            if missing_cols:
                # العثور على أسماء الحالة الأصلية للأعمدة المفقودة لرسالة المستخدم
                ...
                debug_logger.warning(f"الأعمدة المفقودة: {missing_cols_str}")
                flash(f"الملف المرفوع يفتقد للأعمدة المطلوبة: {missing_cols_str}. يرجى التأكد من تنسيق الملف أو تنزيل القالب.", "danger")
                return render_template("question/import_questions.html", lessons=lessons, selected_lesson_id=lesson_id)

        except Exception as e:
            debug_logger.exception(f"خطأ أثناء قراءة أو معالجة الملف المرفوع: {e}")
        ...
        for index, row in df.iterrows():
            row_num = index + 2
            debug_logger.debug(f"معالجة الصف {row_num}...")
            try:
                ...
                debug_logger.debug(f"نص السؤال: {q_text}")
                debug_logger.debug(f"رابط صورة السؤال: {q_image}")
                ...
                for i in range(1, 5):
                    ...
                    debug_logger.debug(f"نص الخيار {i}: {opt_text}")
                    debug_logger.debug(f"رابط صورة الخيار {i}: {opt_image}")
                ...
                debug_logger.debug(f"عدد الخيارات: {len(options_from_row)}")
                
                correct_opt_num_raw = row.get('correct_option_number')
                debug_logger.debug(f"رقم الخيار الصحيح (خام): {correct_opt_num_raw}")
                
                correct_opt_num = -1
                if pd.notna(correct_opt_num_raw):
                    try:
                        correct_opt_num = int(correct_opt_num_raw) - 1
                        debug_logger.debug(f"رقم الخيار الصحيح (معالج): {correct_opt_num}")
                    except (ValueError, TypeError):
                        debug_logger.warning(f"رقم الخيار الصحيح '{correct_opt_num_raw}' غير صالح (يجب أن يكون رقمًا).")
                        error_details.append(f"Row {row_num}: رقم الخيار الصحيح '{correct_opt_num_raw}' غير صالح (يجب أن يكون رقمًا).")
                        continue
                else:
                    debug_logger.warning(f"عمود رقم الخيار الصحيح فارغ.")
                    error_details.append(f"Row {row_num}: عمود رقم الخيار الصحيح فارغ.")
                    continue

                if not q_text and not q_image:
                    debug_logger.warning(f"يجب توفير نص للسؤال أو رابط صورة.")
                    error_details.append(f"Row {row_num}: يجب توفير نص للسؤال أو رابط صورة.")
                    continue
                
                if len(options_from_row) < 2:
                    debug_logger.warning(f"يجب توفير خيارين على الأقل (نص أو صورة).")
                    error_details.append(f"Row {row_num}: يجب توفير خيارين على الأقل (نص أو صورة).")
                    continue

                if not (0 <= correct_opt_num < len(options_from_row)):
                    debug_logger.warning(f"رقم الخيار الصحيح '{correct_opt_num_raw}' غير صالح بالنسبة لعدد الخيارات المتاحة ({len(options_from_row)}).")
                ...
                try:
                    debug_logger.info(f"محاولة إضافة سؤال جديد من الصف {row_num}")
                    new_question = Question(
                    ...
                    db.session.commit()
                    imported_count += 1
                    debug_logger.info(f"تم استيراد السؤال من الصف {row_num} بنجاح (ID: {new_question.question_id})")

                except (IntegrityError, DBAPIError) as db_err:
                    db.session.rollback()
                    debug_logger.error(f"خطأ في قاعدة البيانات عند استيراد الصف {row_num}: {db_err}")
                    error_details.append(f"Row {row_num}: خطأ في قاعدة البيانات - {db_err}")
                    continue
                except Exception as inner_e:
                    db.session.rollback()
                    debug_logger.error(f"خطأ غير متوقع في استيراد الصف {row_num}: {inner_e}", exc_info=True)
                    error_details.append(f"Row {row_num}: خطأ غير متوقع - {inner_e}")
                    continue

            except Exception as outer_e:
                debug_logger.error(f"خطأ في معالجة الصف {row_num}: {outer_e}", exc_info=True)
                error_details.append(f"Row {row_num}: خطأ في معالجة البيانات - {outer_e}")
                continue

        if imported_count > 0:
            debug_logger.info(f"تم استيراد {imported_count} سؤال بنجاح!")
        ...
        if error_details:
            ...
            debug_logger.error(f"أخطاء الاستيراد: {error_details}")
            flash(error_summary, "danger")
            return render_template("question/import_questions.html", lessons=lessons, selected_lesson_id=lesson_id)
        else:
            if imported_count > 0:
                debug_logger.info("تم الانتهاء من الاستيراد بنجاح، جاري التحويل إلى صفحة قائمة الأسئلة")
                return redirect(url_for("question.list_questions"))
            else:
                debug_logger.warning("لم يتم العثور على أسئلة صالحة للاستيراد في الملف.")
                flash("لم يتم العثور على أسئلة صالحة للاستيراد في الملف.", "warning")
                return render_template("question/import_questions.html", lessons=lessons, selected_lesson_id=lesson_id)

    # طلب GET: عرض نموذج الاستيراد
    debug_logger.info("عرض نموذج استيراد الأسئلة (طلب GET)")
    return render_template("question/import_questions.html", lessons=lessons)
...
def delete_question(question_id):
    ...
    try:
        # Delete associated options first (cascade delete should handle this, but being explicit)
