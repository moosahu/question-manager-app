...
def prewarm_fonts(app):
    """
    يُشغَّل مرة واحدة عند startup — يولّد PDF وهمي بكل الخطوط المتاحة
    حتى يخزّنها WeasyPrint في الذاكرة ويسرع الطلبات اللاحقة
    """
    import threading
        ...
        try:
            ...
            for family, filename in font_families.items():
                ...
                if os.path.exists(path):
                    ...
                    font_spans += f"<span style='font-family:{family}'>أ ب ت</span> "

            html_content = f"""<!DOCTYPE html>
<html dir="rtl"><head><meta charset="UTF-8">
<style>{font_faces} body {{ direction: rtl; }}</style>
</head><body>{font_spans}</body></html>"""

            with app.app_context():
                HTML(string=html_content).write_pdf()
            print("✅ WeasyPrint fonts prewarmed successfully")
        except Exception as e:
            print(f"⚠️ Font prewarm failed (non-critical): {e}")

    # في thread منفصل عشان ما يعطّل startup
