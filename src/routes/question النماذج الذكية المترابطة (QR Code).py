...
@question_bp.route('/print-smart-exam-sheets', methods=['POST'])
@login_required  
def print_smart_exam_sheets():
    """طباعة نماذج ذكية مترابطة (ورقة أسئلة + ورقة إجابة) مع QR Code موحد"""
    try:
        import qrcode
        ...
        question_ids = data.get('question_ids', [])
        shuffle_options = data.get('shuffle_options', True)
        output_type = data.get('output_type', 'both')  # 'both', 'exam_only', 'answer_only'
        ...
        all_html = ""
        global_sequence = 1  # رقم تسلسلي عام
        
        # توليد النماذج لكل نموذج
        for idx, model_letter in enumerate(models):
            ...
            for i in range(count_per_model):
                # إنشاء معرف فريد
                unique_id = f"{model_letter}{global_sequence:03d}"
                
                # إنشاء QR Code
                qr_data_str = f"ID:{unique_id}|Model:{model_letter}|Seq:{global_sequence}|Date:{datetime.now().strftime('%Y-%m-%d')}|Exam:{header_context['subject']}"
                ...
                # طباعة ورقة الأسئلة
                if output_type in ['both', 'exam_only']:
                ...
                # طباعة ورقة الإجابة
                if output_type in ['both', 'answer_only']:
                    answer_sheet_html = render_template(
                        'question/remark_answer_sheet.html',
                        student={
                            'name': '..........................................',
        ...
        current_app.logger.error(f"Error in print_smart_exam_sheets: {str(e)}")
        import traceback
        traceback.print_exc()
        return jsonify({'error': f'فشل تجهيز الطباعة: {str(e)}'}), 500
