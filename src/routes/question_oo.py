# src/routes/question.py (Modified to fix import issue)
...
# --- import_questions route (Modified to fix import issue) --- #
@question_bp.route("/import", methods=["GET", "POST"])
@login_required
def import_questions():
    """Import questions from Excel or CSV file."""
    current_app.logger.info("Entering import_questions function")
    ...
    if request.method == "POST":
        ...
        # Check if file was uploaded - FIXED: Changed from import_file to question_file
        current_app.logger.info(f"Request files keys: {list(request.files.keys())}")
        
        if "question_file" not in request.files:  # FIXED: Changed from import_file to question_file
        ...
        import_file = request.files["question_file"]  # FIXED: Changed from import_file to question_file
        if not import_file or not import_file.filename:
            flash("لم يتم تحديد ملف للاستيراد.", "danger")
            return render_template("question/import_questions.html", lessons=lessons, selected_lesson_id=lesson_id)
        
        current_app.logger.info(f"File received: {import_file.filename}")
...
# --- download_template route (keep as is) --- #
